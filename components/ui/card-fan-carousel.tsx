...
export interface CardItem {
  ...
  alt?: string;
  linkUrl?: string;
  /** Small uppercase tag shown at the bottom of the card (e.g. a category). */
  eyebrow?: string;
  /** Main caption line shown below the eyebrow. */
  caption?: string;
}
...
// Show 3 cards at a time (left / center / right) with prev/next arrows to
// bring the rest into view, instead of fanning every card out at once.
const MAX_VISIBLE = 3;
const HALF = 1;

const FAN_POSITIONS = [
  { rot: -14, scale: 0.86, x: -20, y: 3.4, zIndex: 2 },
...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 overflow-x-hidden">
          ...
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden">
                ...
                {(card.eyebrow || card.caption) && (
                  <div className="absolute inset-x-0 bottom-0 z-20 px-4 py-3 md:px-5 md:py-4 bg-gradient-to-t from-black/85 via-black/45 to-transparent">
                    {card.eyebrow && (
                      <span className="block text-[8px] md:text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--accent,#e9a93c)]">
                        {card.eyebrow}
                      </span>
                    )}
                    {card.caption && (
                      <span className="block mt-0.5 text-[10px] md:text-sm font-bold leading-tight text-white">
                        {card.caption}
                      </span>
                    )}
                  </div>
