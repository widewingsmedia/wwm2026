...
function generateHtaccess(redirects: { from: string; to: string; type: '301' | '302' }[]): string {
  const lines = [
    '# Wide Wings Media — 301/302 Redirects',
    '# Generated by SEO Manager admin panel — paste into your .htaccess file',
    '',
  ];
...
function SeoEditorInner() {
  ...
  const [showHtaccess, setShowHtaccess] = useState(false);
  const [seoTools, setSeoTools] = useState<SeoTools | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/seo-tools').then(r => r.ok ? r.json() : null).then(data => data && setSeoTools(data));
  }, []);
  ...
  return (
    <AdminShell session={fakeSession} title="SEO Manager" subtitle="Manage on-page SEO, metadata, redirects, sitemap, robots.txt and llms.txt">
          ...
          {redirects.length > 0 && (
            <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid #f3f4f6' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                <div>
                  <div style={{ fontSize: '0.88rem', fontWeight: 700, color: '#374151' }}>.htaccess Redirects</div>
                  <div style={{ fontSize: '0.78rem', color: '#9ca3af', marginTop: 2 }}>
                    Apache-format 301/302 rules for the {redirects.length} redirect{redirects.length === 1 ? '' : 's'} above — paste into wide-wings.ae&apos;s .htaccess file.
                  </div>
                </div>
                <button className="adm-btn adm-btn-outline adm-btn-sm" onClick={() => setShowHtaccess(v => !v)}>
                  {showHtaccess ? 'Hide' : 'Generate .htaccess'}
                </button>
              </div>
              {showHtaccess && (
                <div style={{ marginTop: 14 }}>
                  <pre style={{ background: '#0B0B0E', color: '#e5e7eb', padding: 16, borderRadius: 8, fontSize: '0.8rem', overflowX: 'auto', fontFamily: 'monospace', lineHeight: 1.6 }}>
{generateHtaccess(redirects)}
                  </pre>
                  <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                    <CopyButton text={generateHtaccess(redirects)} />
                    <button className="adm-btn adm-btn-outline adm-btn-sm" onClick={() => downloadFile('.htaccess', generateHtaccess(redirects))}>
                      Download .htaccess
                    </button>
                  </div>
                </div>
      ...
      {/* SEO TOOLS: SITEMAP / ROBOTS.TXT / LLMS.TXT */}
      <div className="adm-card">
        <div className="adm-card-head"><div className="adm-card-title">XML Sitemap</div></div>
        <div style={{ padding: 24 }}>
          <p style={{ fontSize: '0.88rem', color: '#6b7280', marginBottom: 16 }}>
            Auto-generated from all published pages, service pages, case studies, and blog posts. Updates automatically — no manual regeneration needed.
          </p>
          {seoTools && !seoTools.isLive && (
            <p style={{ fontSize: '0.82rem', color: '#f59e0b', marginBottom: 16 }}>
              Site is not live yet (see robots.txt below) — the sitemap serves an empty list until <code style={{ background: '#f3f4f6', padding: '2px 6px', borderRadius: 4 }}>NEXT_PUBLIC_SITE_LIVE=true</code> is set at go-live.
            </p>
          )}
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            <input className="adm-input" readOnly value={seoTools?.sitemapUrl ?? ''} style={{ flex: 1, minWidth: 240, fontFamily: 'monospace', fontSize: '0.82rem', background: '#f9fafb' }} />
            {seoTools && <CopyButton text={seoTools.sitemapUrl} />}
            <a className="adm-btn adm-btn-primary adm-btn-sm" href="/sitemap.xml" target="_blank" rel="noopener" style={{ textDecoration: 'none' }}>View Sitemap</a>
          </div>
        </div>
      </div>

      <div className="adm-card">
        <div className="adm-card-head"><div className="adm-card-title">robots.txt</div></div>
        <div style={{ padding: 24 }}>
          <p style={{ fontSize: '0.88rem', color: '#6b7280', marginBottom: 16 }}>
            Current live ruleset served at <code style={{ background: '#f3f4f6', padding: '2px 6px', borderRadius: 4 }}>/robots.txt</code>
            {seoTools && (
              <> — site is currently <strong style={{ color: seoTools.isLive ? '#16a34a' : '#f59e0b' }}>{seoTools.isLive ? 'LIVE (crawling allowed)' : 'NOT LIVE (crawling blocked)'}</strong>.</>
            )}
          </p>
          <pre style={{ background: '#0B0B0E', color: '#e5e7eb', padding: 16, borderRadius: 8, fontSize: '0.8rem', overflowX: 'auto', fontFamily: 'monospace', lineHeight: 1.6 }}>
{seoTools?.robotsTxt ?? 'Loading…'}
          </pre>
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            {seoTools && <CopyButton text={seoTools.robotsTxt} />}
            <a className="adm-btn adm-btn-outline adm-btn-sm" href="/robots.txt" target="_blank" rel="noopener" style={{ textDecoration: 'none' }}>View Live</a>
          </div>
        </div>
      </div>

      <div className="adm-card">
        <div className="adm-card-head"><div className="adm-card-title">llms.txt</div></div>
        <div style={{ padding: 24 }}>
          <p style={{ fontSize: '0.88rem', color: '#6b7280', marginBottom: 16 }}>
            Machine-readable site overview for AI assistants and LLM-based search (ChatGPT, Perplexity, Claude, etc.), served at <code style={{ background: '#f3f4f6', padding: '2px 6px', borderRadius: 4 }}>/llms.txt</code>. Same go-live gate as robots.txt — this preview always shows the current content.
          </p>
          <pre style={{ background: '#0B0B0E', color: '#e5e7eb', padding: 16, borderRadius: 8, fontSize: '0.8rem', overflowX: 'auto', fontFamily: 'monospace', lineHeight: 1.6, maxHeight: 360, overflowY: 'auto' }}>
{seoTools?.llmsTxt ?? 'Loading…'}
          </pre>
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            {seoTools && <CopyButton text={seoTools.llmsTxt} />}
            {seoTools && <button className="adm-btn adm-btn-outline adm-btn-sm" onClick={() => downloadFile('llms.txt', seoTools.llmsTxt)}>Download llms.txt</button>}
            <a className="adm-btn adm-btn-outline adm-btn-sm" href="/llms.txt" target="_blank" rel="noopener" style={{ textDecoration: 'none' }}>View Live</a>
          </div>
        </div>
      </div>
